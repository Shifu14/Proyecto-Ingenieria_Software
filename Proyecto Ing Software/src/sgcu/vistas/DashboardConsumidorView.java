...
public class DashboardConsumidorView extends JFrame {
    ...
    private JLabel lblBienvenida, lblSaldoValor, lblAvisoBloqueo; 
    private JButton btnMenu, btnSaldo, btnReservas, btnHistorial, btnLogout, btnVerificarAcceso;
    private JPanel panelCentralMensaje;
    ...
    private void inicializarComponentes() {
        ...
        // PANEL DE AVISO DE BLOQUEO
        //  Centra el panel del mensaje y botón de bloqueo
        panelCentralMensaje = new JPanel(new FlowLayout(FlowLayout.CENTER, 15, 0));
        panelCentralMensaje.setOpaque(false);
        
        lblAvisoBloqueo = new JLabel("<html><center>Para acceder a todas las funciones del comedor es necesario acceder<br>al sistema de reconocimiento facial y cobro de tarifa.</center></html>");
        lblAvisoBloqueo.setFont(new Font("Segoe UI", Font.BOLD, 16));
        lblAvisoBloqueo.setForeground(new Color(220, 53, 69)); // Rojo
        
        btnVerificarAcceso = crearBotonEstilizado("Verificar Identidad (500 Bs)", new Color(0, 123, 255));
        btnVerificarAcceso.setPreferredSize(new Dimension(250, 40));
        
        panelCentralMensaje.add(lblAvisoBloqueo);
        panelCentralMensaje.add(btnVerificarAcceso);

        contentBox.add(Box.createVerticalStrut(20)); // Espacio superior
        contentBox.add(lblBienvenida); 
        contentBox.add(Box.createVerticalStrut(30)); // Espacio entre bienvenida y aviso
        contentBox.add(panelCentralMensaje); 
        contentBox.add(Box.createVerticalStrut(40)); // Espacio antes de las tarjetas
    ...
    // MÉTODO PARA BLOQUEAR O DESBLOQUEAR LA UI
    public void aplicarBloqueo(boolean concedido) {
        panelCentralMensaje.setVisible(!concedido); 
        
        Color colorBloqueado = new Color(20, 20, 20); 
        Color colorActivo = new Color(80, 80, 80); 
        
        btnMenu.setBackground(concedido ? colorActivo : colorBloqueado);
        btnReservas.setBackground(concedido ? colorActivo : colorBloqueado);
        btnHistorial.setBackground(concedido ? colorActivo : colorBloqueado);
    }
    ...
    public void addVerificarAccesoListener(ActionListener l) { btnVerificarAcceso.addActionListener(l); }
    ...
    public void mostrarErrorBloqueo() {
        JOptionPane.showMessageDialog(this, "Primero debe verificar su identidad y pagar la tarifa.", "Acceso Denegado", JOptionPane.WARNING_MESSAGE);
    }
    ...
    private JButton crearBotonSidebar(String txt) {
        ...
        btn.setOpaque(true); btn.setBackground(new Color(80, 80, 80)); 
