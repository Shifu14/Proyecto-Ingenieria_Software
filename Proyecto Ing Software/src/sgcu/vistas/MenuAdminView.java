...
public class MenuAdminView extends JFrame {
    ...
    public MenuAdminView() {
        ...
        txtId = new JTextField(); txtId.setEditable(false); txtId.setBackground(new Color(220, 220, 220));
        ...
        agregarCampoFormulario(panelFormulario, "Precio (Bs):", txtPrecio, 3);
        ...
        g.gridx = 0; g.gridy = 6; g.gridwidth = 2; g.fill = GridBagConstraints.HORIZONTAL; g.insets = new Insets(20,0,0,0);
        panelFormulario.add(panelAcciones, g);
        ...
        String[] columnas = {"ID", "Nombre", "Descripción", "Precio", "Día", "Servicio"};
    ...
    private void agregarCampoFormulario(JPanel p, String label, JComponent f, int y) {
        ...
        if (f instanceof JTextField) f.setBackground(new Color(250,250,250));
        GridBagConstraints g = new GridBagConstraints();
        g.gridx=0; g.gridy=y; g.anchor=GridBagConstraints.WEST; g.insets=new Insets(5,0,5,5);
        p.add(lbl, g);
        g.gridx=1; g.gridy=y; g.fill=GridBagConstraints.HORIZONTAL; g.weightx=1.0;
        p.add(f, g);
    }

    private void estilizarCombo(JComboBox<?> c) { c.setFont(new Font("Segoe UI", Font.PLAIN, 14)); c.setBackground(Color.WHITE); }
    private void estilizarTabla(JTable t) {
        t.setFont(new Font("Segoe UI", Font.PLAIN, 14)); t.setRowHeight(35);
        t.getTableHeader().setBackground(new Color(41, 41, 41));
        t.getTableHeader().setForeground(Color.WHITE);
    }

    // --- CAMBIO APLICADO: BOTÓN DINÁMICO ---
    private JButton crearBotonEstilizado(String t, Color bg) {
        JButton b = new JButton(t) {
            protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                Color colorActual = getBackground(); // Lee el color en tiempo real
    ...
    public void agregarFilaTabla(Object[] fila) { modeloTabla.addRow(fila); }
    public void limpiarFormulario() {
        txtId.setText(""); txtNombre.setText(""); txtDescripcion.setText(""); txtPrecio.setText("");
        comboDias.setSelectedIndex(0); comboServicio.setSelectedIndex(0);
    ...
    // --- NUEVOS MÉTODOS PARA EL MODO EDICIÓN ---
    public void setModoEdicion(boolean activado) {
        if (activado) {
            btnEditar.setText("GUARDAR");
            btnEditar.setBackground(new Color(40, 167, 69)); // Verde
        } else {
            btnEditar.setText("EDITAR");
            btnEditar.setBackground(new Color(255, 193, 7)); // Amarillo (el original)
        }
        btnEditar.repaint(); 
    }
